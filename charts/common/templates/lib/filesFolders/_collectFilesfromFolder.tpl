{{- define "retsamedoc.common.lib.filesFolders.collectFilesfromFolder" -}}
  ...
  {{- range $folder := $folders -}}
    ...
    {{- range $file_name, $content := $allFilesContent -}}
      ...
      {{- if not $fileOverride.exclude -}}
        ...
        {{- else -}}
          {{- $isBinary = eq (include "retsamedoc.common.lib.filesFolders.isBinaryFile" (dict "rootContext" $rootContext "file" $file_name)) "true" -}}
        {{- end -}}
        ...
        {{- else if $fileOverride.isEnvFile -}}
          ...
          {{- $parsedEnv := include "retsamedoc.common.lib.filesFolders.parseEnvFile" (dict "content" $fileContent) | fromYaml -}}
