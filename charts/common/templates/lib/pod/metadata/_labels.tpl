...
*/ -}}
{{- define "retsamedoc.common.lib.pod.metadata.labels" -}}
  ...
  {{- if $rootContext.Values.global.propagateGlobalMetadataToPods -}}
    {{- $labels = merge
      (include "retsamedoc.common.lib.metadata.globalLabels" $rootContext | fromYaml)
      $labels
    -}}
  {{- end -}}

  {{- /* Fetch the Pod selectorLabels */ -}}
  {{- $selectorLabels := include "retsamedoc.common.lib.metadata.selectorLabels" $rootContext | fromYaml -}}
  ...
  {{- $podlabels := (include "retsamedoc.common.lib.pod.getOption" (dict "ctx" $ctx "option" "labels")) | fromYaml -}}
