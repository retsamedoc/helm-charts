...
*/ -}}
{{- define "retsamedoc.common.lib.pod.field.serviceAccountName" -}}
  ...
  {{- if not (has "serviceAccount" (keys $controllerObject)) -}}
    {{- if (eq (len $enabledServiceAccounts) 1) -}}
      ...
      {{- $subject := (include "retsamedoc.common.lib.serviceAccount.getByIdentifier" (dict "rootContext" $rootContext "id" $saIdentifier) | fromYaml) -}}
      {{- $serviceAccountName = get $subject "name" -}}
    {{- end -}}
  {{- else -}}
    {{- if hasKey $controllerObject.serviceAccount "identifier" -}}
      {{- $subject := (include "retsamedoc.common.lib.serviceAccount.getByIdentifier" (dict "rootContext" $rootContext "id" $controllerObject.serviceAccount.identifier) | fromYaml) -}}
