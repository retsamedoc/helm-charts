...
*/}}
{{- define "retsamedoc.common.lib.chart.validate" -}}
  {{- $rootContext := . -}}
  ...
  {{- range $persistenceKey, $persistenceValues := .Values.persistence }}
    ...
    {{- if $persistenceEnabled -}}
      ...
      {{- range $key, $advancedMount := $persistenceValues.advancedMounts -}}
          {{- $mountController := include "retsamedoc.common.lib.controller.getByIdentifier" (dict "rootContext" $rootContext "id" $key) -}}
