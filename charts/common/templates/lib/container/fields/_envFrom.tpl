...
*/}}
{{- define "retsamedoc.common.lib.container.field.envFrom" -}}
  {{- $ctx := .ctx -}}
  ...
  {{- if not (empty $envFromValues) -}}
    ...
    {{- range $envFromValues -}}
      ...
      {{- if hasKey . "configMap" -}}
        {{- $configMap := include "retsamedoc.common.lib.configMap.getByIdentifier" (dict "rootContext" $rootContext "id" .configMap) | fromYaml -}}
      ...
      {{- else if hasKey . "configMapRef" -}}
        {{- if not (empty (dig "identifier" nil .configMapRef)) -}}
          {{- $configMap := include "retsamedoc.common.lib.configMap.getByIdentifier" (dict "rootContext" $rootContext "id" .configMapRef.identifier) | fromYaml -}}
          {{- if empty $configMap -}}
      ...
      {{- else if hasKey . "secret" -}}
        {{- $secret := include "retsamedoc.common.lib.secret.getByIdentifier" (dict "rootContext" $rootContext "id" .secret) | fromYaml -}}
      ...
      {{- else if hasKey . "secretRef" -}}
        {{- if not (empty (dig "identifier" nil .secretRef)) -}}
          {{- $secret := include "retsamedoc.common.lib.secret.getByIdentifier" (dict "rootContext" $rootContext "id" .secretRef.identifier) | fromYaml -}}
          {{- if empty $secret -}}
