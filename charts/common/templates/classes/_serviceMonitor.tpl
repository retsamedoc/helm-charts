{{- define "retsamedoc.common.class.serviceMonitor" -}}
  ...
  {{ $service := dict -}}
  {{ $serviceName := "" -}}
  {{ if not (empty (dig "service" "name" nil $serviceMonitorObject)) -}}
  ...
  {{ else if not (empty (dig "service" "identifier" nil $serviceMonitorObject)) -}}
    {{ $service = (include "retsamedoc.common.lib.service.getByIdentifier" (dict "rootContext" $rootContext "id" $serviceMonitorObject.service.identifier) | fromYaml ) -}}
    {{ if not $service -}}
...
spec:
  jobLabel: {{ include "retsamedoc.common.lib.serviceMonitor.field.jobLabel" (dict "ctx" $ctx) | trim }}
  namespaceSelector:
    matchNames:
